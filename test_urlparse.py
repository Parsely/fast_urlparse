...
class UrlParseTestCase(unittest.TestCase):
    ...
    def test_default_scheme(self):
        ...
        for func in (urlparse.urlparse, urlparse.urlsplit):
    ...
    def test_parse_fragments(self):
        ...
        for url, attr in tests:
            for func in (urlparse.urlparse, urlparse.urlsplit):
                if attr == "params" and func is urlparse.urlsplit:
                    attr = "path"
    ...
    def test_parse_qs_encoding(self):
        result = urlparse.parse_qs("key=\u0141%E9", encoding="latin-1")
        self.assertEqual(result, {'key': ['\u0141\xE9']})
        result = urlparse.parse_qs("key=\u0141%C3%A9", encoding="utf-8")
        self.assertEqual(result, {'key': ['\u0141\xE9']})
        result = urlparse.parse_qs("key=\u0141%C3%A9", encoding="ascii")
        self.assertEqual(result, {'key': ['\u0141\ufffd\ufffd']})
        result = urlparse.parse_qs("key=\u0141%E9-", encoding="ascii")
        self.assertEqual(result, {'key': ['\u0141\ufffd-']})
        result = urlparse.parse_qs("key=\u0141%E9-", encoding="ascii",
                                                          errors="ignore")
        self.assertEqual(result, {'key': ['\u0141-']})
    ...
    def test_parse_qsl_encoding(self):
        result = urlparse.parse_qsl("key=\u0141%E9", encoding="latin-1")
        self.assertEqual(result, [('key', '\u0141\xE9')])
        result = urlparse.parse_qsl("key=\u0141%C3%A9", encoding="utf-8")
        self.assertEqual(result, [('key', '\u0141\xE9')])
        result = urlparse.parse_qsl("key=\u0141%C3%A9", encoding="ascii")
        self.assertEqual(result, [('key', '\u0141\ufffd\ufffd')])
        result = urlparse.parse_qsl("key=\u0141%E9-", encoding="ascii")
        self.assertEqual(result, [('key', '\u0141\ufffd-')])
        result = urlparse.parse_qsl("key=\u0141%E9-", encoding="ascii",
                                                          errors="ignore")
        self.assertEqual(result, [('key', '\u0141-')])
    ...
    def test_urlencode_sequences(self):
        ...
        result = urlparse.urlencode({'a': [1, 2], 'b': (3, 4, 5)}, True)
        ...
        result = urlparse.urlencode({'a': Trivial()}, True)
    ...
    def test_quote_from_bytes(self):
        self.assertRaises(TypeError, urlparse.quote_from_bytes, 'foo')
    ...
    def test_unquote_to_bytes(self):
        result = urlparse.unquote_to_bytes('abc%20def')
        self.assertEqual(result, b'abc def')
        result = urlparse.unquote_to_bytes('')
    ...
    def test_all(self):
        ...
        for name in dir(urlparse):
            if name.startswith('_') or name in undocumented:
                continue
            object = getattr(urlparse, name)
            if getattr(object, '__module__', None) == 'urlparse':
                expected.append(name)
        self.assertCountEqual(urlparse.__all__, expected)
...
class Utility_Tests(unittest.TestCase):
    ...
    def test_splitpasswd(self):
        ...
        splitpasswd = urlparse.splitpasswd
    ...
    def test_splitnport(self):
        splitnport = urlparse.splitnport
    ...
    def test_splitquery(self):
        ...
        splitquery = urlparse.splitquery
    ...
    def test_splitvalue(self):
        ...
        splitvalue = urlparse.splitvalue
    ...
    def test_to_bytes(self):
        result = urlparse.to_bytes('http://www.python.org')
        self.assertEqual(result, 'http://www.python.org')
        self.assertRaises(UnicodeError, urlparse.to_bytes,
                          'http://www.python.org/medi\u00e6val')

    def test_unwrap(self):
        url = urlparse.unwrap('<URL:type://host/path>')
